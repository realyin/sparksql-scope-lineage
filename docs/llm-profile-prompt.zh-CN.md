...
14. 详细还原模式下，每个加工阶段必须绑定 `scope_profile.steps[].name`，不能只写
    “第一步、第二步”或只写业务标题。scope 名称是后续回查 SQL、profile.json 和
    lineage.json 的定位锚点。
15. 详细还原模式下，每个加工阶段必须说明关键条件表达式或条件摘要。不要只列
    “涉及字段”。如果 profile 只提供字段而没有完整条件，必须明确写“profile 中未提供
    完整条件，只能根据字段推断”。
16. 每条业务规则必须说明“字段在规则里的作用”：字段英文名、字段中文名/注释、
    条件表达式、字段如何参与判断、满足条件后的处理结果。不要输出无法追溯的
    字段清单式规则。
```
...
- 每张关键输入表必须同时写英文表名、中文表名/表说明、在本任务中的作用；
  如果中文元数据缺失，要写“中文名缺失，只能按英文名/字段推断”
- 每张关键输入表必须列出核心使用字段，字段说明格式为：
  字段英文名 / 字段中文名或注释 / 在本任务中的作用
- 输出表必须写英文表名、中文表名/表说明、输出业务对象；
  输出字段必须尽量同时写英文名和中文含义，不能只写中文名或只写英文名
...
- 每个关键步骤必须按以下格式输出：
  - 阶段名称：根据 step.name 和业务动作生成，不要只写“步骤1”
  - scope：必须原样写出 step.name
  - scope 类型：来自 step.kind
  - 技术角色：来自 step.role，但不能把它当业务结论
  - 输入：物理源表或上游 scope
  - 关键条件：来自 business_rule_candidates / logic.filters / joins.on，
    必须写出条件表达式或可读条件摘要，不允许只列字段名
  ...
  - 关键字段关系：列出本阶段最重要字段的英文名、中文名/注释，以及字段在条件、
    关联、聚合、窗口或派生中的作用

L3.5：业务规则/判断逻辑
- 必须单独列出从 business_rule_candidates 和 filters_summary 归纳出的规则组
- 每个规则组必须包含：规则名称、所在 scope、条件表达式或条件摘要、涉及字段、
  字段中文语义、字段在规则中的判断作用、满足条件后的处理结果
- 不要只写“按过滤条件筛选”，要解释筛选条件在业务上可能表示什么
- 对复杂 OR/AND 条件，要尽量拆成多个业务规则组，例如：
  准入条件、排除条件、时间窗口、状态判断、金额阈值、名单命中、去重取最新、渠道合并
- 对每个字段，必须解释它和规则的关系，例如：
  “dpd 表示逾期天数，在 `dpd between -7 and 0` 中用于判断是否进入提前预警窗口”，
  而不是只写“涉及字段：dpd”
...
- 输出字段说明必须尽量包含：输出字段英文名、中文名/注释、来源字段英文名、
  来源字段中文名/注释、加工方式、业务作用
- 如果字段参与 L3.5 的规则判断，要说明它参与了哪条规则，以及在规则中承担的是
  标识、时间窗口、状态判断、金额阈值、名单命中、分类派生还是指标计算作用
- 详细还原模式下，至少覆盖：
  - 候选粒度字段 grain.keys
...
- 详细还原模式下，每个 L3 加工阶段是否都写出了 `scope` 名称；
- 每个关键加工阶段是否写出了关键条件表达式或条件摘要，而不是只列字段；
- L3.5 每条业务规则是否写出了所在 scope、条件、字段语义、字段在规则中的作用、
  满足条件后的处理结果；
- 输入表和输出表是否同时保留英文表名和中文表名/说明，关键字段是否同时保留
  英文名和中文注释；
